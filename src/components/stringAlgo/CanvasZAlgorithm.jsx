...
export const CanvasZAlgorithm = ({ text, pattern, speed }) => {
  ...
  useEffect(() => {
    if (!text || !pattern) {
      setZArray([]); setActiveI(-1); setMatches([]); return
    }
  ...
  return (
    <div className="w-full">
      <div className="rounded-xl border border-white/10 bg-slate-900/50 p-6 shadow-lg min-h-[380px] flex flex-col gap-6">
        ...
        {zArray.length > 0 && (
          <div>
            <p className="text-xs text-slate-500 uppercase tracking-widest mb-2">Z-Array (text portion)</p>
            <div className="flex flex-wrap gap-2">
              {text.split('').map((_, i) => (
                <div key={i} className="flex flex-col items-center">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold border-2 transition-all duration-300
                    ${i === activeI
