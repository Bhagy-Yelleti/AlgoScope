...
import { MathComparisonMode } from './MathComparisonMode'
...
function ComparePanel({ label, steps, speed, accent, children }) {
  ...
  useEffect(() => {
    if (!hasSteps) { setStepIndex(-1); setIsFinished(false); return }
    setStepIndex(0)
    setIsFinished(false)
  }, [steps])

  useEffect(() => {
    if (!hasSteps || stepIndex < 0 || stepIndex >= steps.length - 1 || isFinished) return
...
export const MathComparisonMode = ({ speed }) => {
  ...
  const { leftSteps, rightSteps, leftLabel, rightLabel, leftAccent, rightAccent } = useMemo(() => {
    if (trigger === 0) return { leftSteps: [], rightSteps: [], leftLabel: '', rightLabel: '', leftAccent: '#06b6d4', rightAccent: '#a855f7' }
    ...
    return {
      ...
      rightSteps: generateBitOpSteps(Number(bitA), Number(bitB), bitOp === 'AND' ? 'OR' : 'AND'),
  ...
  return (
    <div className="p-4 sm:p-6 flex flex-col gap-6">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div className="flex gap-1 rounded-xl p-1" style={{ background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(51,65,85,0.6)' }}>
          {COMPARE_TABS.map((t) => (
            <button
              key={t.key}
              onClick={() => { setTab(t.key); setTrigger(0) }}
            ...
            >
              {tab === t.key && (
                <motion.div layoutId="compare-tab-bg"
      ...
      <div className="flex flex-wrap items-end gap-4 p-4 rounded-xl bg-slate-900/50 border border-white/5">
        {tab === 'gcd' && (
          <>
            <div>
              ...
              <input type="number" min={1} max={9999} value={gcdA}
            ...
            <div>
              ...
              <input type="number" min={1} max={9999} value={gcdB}
        ...
        {tab === 'expo' && (
          <>
            <div>
              ...
              <input type="number" min={2} max={20} value={expoBase}
            ...
            <div>
              <label className="text-xs text-slate-500 mb-1 block">Exponent (≤ 15)</label>
              <input type="number" min={1} max={15} value={expoExp}
        ...
        {tab === 'bits' && (
          <>
            <div>
              <label className="text-xs text-slate-500 mb-1 block">A</label>
              <input type="number" min={0} max={255} value={bitA}
                onChange={(e) => setBitA(Math.min(255, Math.max(0, e.target.value)))}
                className="w-24 rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" />
            </div>
            <div>
              <label className="text-xs text-slate-500 mb-1 block">B</label>
              <input type="number" min={0} max={255} value={bitB}
                onChange={(e) => setBitB(Math.min(255, Math.max(0, e.target.value)))}
                className="w-24 rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" />
            </div>
            <div>
              <label className="text-xs text-slate-500 mb-1 block">Primary Op</label>
              <select value={bitOp} onChange={(e) => setBitOp(e.target.value)}
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        <ComparePanel label={leftLabel || '—'} steps={leftSteps} speed={speed} accent={leftAccent}>
          {({ currentStep, hasSteps }) => tab === 'gcd' ? (
            <CanvasGCD currentStep={currentStep} hasSteps={hasSteps} inputA={Number(gcdA)} inputB={Number(gcdB)} />
          ) : tab === 'expo' ? (
            <CanvasFastExpo currentStep={currentStep} hasSteps={hasSteps} inputBase={Number(expoBase)} inputExp={Number(expoExp)} />
          ) : (
            <CanvasBitManip currentStep={currentStep} hasSteps={hasSteps} inputA={Number(bitA)} inputB={Number(bitB)} operation={bitOp} />
          )}
        </ComparePanel>

        <ComparePanel label={rightLabel || '—'} steps={rightSteps} speed={speed} accent={rightAccent}>
          {({ currentStep, hasSteps }) => tab === 'gcd' ? (
            <CanvasGCD currentStep={currentStep} hasSteps={hasSteps} inputA={Number(gcdA)} inputB={Number(gcdB)} />
          ) : tab === 'expo' ? (
            <CanvasFastExpo currentStep={currentStep} hasSteps={hasSteps} inputBase={Number(expoBase)} inputExp={Number(expoExp)} />
          ) : (
            <CanvasBitManip currentStep={currentStep} hasSteps={hasSteps}
              inputA={Number(bitA)} inputB={Number(bitB)}
      ...
      {trigger > 0 && leftSteps.length > 0 && (
        <div className="rounded-xl bg-slate-900/50 border border-white/5 p-5">
          <p className="text-xs text-slate-500 uppercase tracking-widest mb-3">Step Count Comparison</p>
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 rounded-xl bg-slate-800/60 p-4 border border-cyan-500/20 text-center">
              ...
              <h2 className="text-3xl font-bold text-cyan-400 mt-1">{leftSteps.length}</h2>
            ...
            <div className="flex-1 rounded-xl bg-slate-800/60 p-4 border border-purple-500/20 text-center">
              ...
              <h2 className="text-3xl font-bold text-purple-400 mt-1">{rightSteps.length}</h2>
              <p className="text-slate-500 text-xs mt-1">steps</p>
            </div>
