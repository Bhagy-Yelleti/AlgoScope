...
export const CanvasBitManip = ({ currentStep, hasSteps, inputA, inputB, operation }) => {
  const activeIndices = currentStep?.indices ?? []
  const isComplete = currentStep?.type === 'complete'
  const message = currentStep?.message ?? 'Enter values, choose an operation, then click Visualize.'
  ...
  const getResult = () => {
    const a = inputA ?? 42, b = inputB ?? 15
    switch (operation) {
      case 'AND': return toBits(a & b)
      case 'OR':  return toBits(a | b)
      case 'XOR': return toBits(a ^ b)
      case 'NOT': return toBits((~a) & 0xff)
      case 'LSHIFT': return toBits((a << 1) & 0xff)
      case 'RSHIFT': return toBits(a >> 1)
      default: return toBits(a & b)
    }
  }

  const bitsResult = isComplete ? (currentStep?.array ?? getResult()) : getResult()
  ...
  const resultValue = (() => {
    ...
    switch (operation) {
      ...
      case 'RSHIFT': return a >> 1
      default: return a & b
    }
  })()

  const BitRow = ({ bits, label, color, highlight }) => (
  ...
  const opSymbols = { AND: '&', OR: '|', XOR: '^', NOT: '~', LSHIFT: '<<', RSHIFT: '>>' }
  ...
  return (
    <div className="w-full">
      <div className="rounded-xl border border-white/10 bg-slate-900/50 p-8 shadow-lg min-h-[350px] flex flex-col justify-center gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-1.5">
            {Array.from({ length: 8 }, (_, i) => (
              <div key={i} className="w-10 text-center text-[10px] text-slate-600 font-mono">
                {7 - i}
              </div>
            ))}
          </div>
        ...
        <BitRow bits={bitsA} label="A" color="text-cyan-400" highlight="a" />

        {operation !== 'NOT' && operation !== 'LSHIFT' && operation !== 'RSHIFT' && (
          <BitRow bits={bitsB} label="B" color="text-purple-400" highlight="b" />
        )}
        ...
        <BitRow bits={bitsResult} label="=" color="text-emerald-400" highlight="result" />

        {/* Result card */}
        <div className="grid grid-cols-3 gap-4 mt-4">
          <div className="rounded-xl bg-slate-800/60 p-4 border border-slate-700">
            <p className="text-slate-400 text-xs">A</p>
            <h2 className="text-2xl font-bold text-cyan-400 mt-1 font-mono">{inputA ?? 42}</h2>
          </div>
          {operation !== 'NOT' && operation !== 'LSHIFT' && operation !== 'RSHIFT' ? (
            <div className="rounded-xl bg-slate-800/60 p-4 border border-slate-700">
              <p className="text-slate-400 text-xs">B</p>
              <h2 className="text-2xl font-bold text-purple-400 mt-1 font-mono">{inputB ?? 15}</h2>
            </div>
          ) : <div />}
          <div className="rounded-xl bg-slate-800/60 p-4 border border-slate-700">
            <p className="text-slate-400 text-xs">Result</p>
            <h2 className="text-2xl font-bold text-emerald-400 mt-1 font-mono">{resultValue}</h2>
          </div>
        </div>
      </div>
