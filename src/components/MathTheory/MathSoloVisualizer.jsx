...
export const MathSoloVisualizer = ({ speed, language }) => {
  ...
  const { currentStep, hasSteps, isPlaying, isComplete, loadSteps, clear, play, pause, replay, stepForward } =
  ...
  const activeComplexityKey = ALGO_TABS.find(t => t.key === algo)?.complexityKey
      ...
      <div className="w-full lg:w-1/4 xl:w-1/5 p-4 flex flex-col gap-5 bg-slate-900/80 shadow-xl rounded-xl border border-white/5 backdrop-blur-sm">
        ...
        {algo === 'gcd' && (
          <div className="space-y-3">
            ...
            <Tooltip content="Second number" position="right" className="w-full">
              <div>
        ...
        {algo === 'expo' && (
          <div className="space-y-3">
            ...
            <Tooltip content="Exponent (keep ≤ 15 for readability)" position="right" className="w-full">
              <div>
                <label className="text-xs text-slate-500 mb-1 block">Exponent</label>
        ...
        {algo === 'bits' && (
          <div className="space-y-3">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Inputs</p>
            <Tooltip content="First operand (0–255)" position="right" className="w-full">
              <div>
                <label className="text-xs text-slate-500 mb-1 block">A (0–255)</label>
                <input type="number" min={0} max={255} value={bitA}
                  onChange={(e) => setBitA(Math.min(255, Math.max(0, e.target.value)))}
                  className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-white outline-none focus:border-cyan-500 text-sm" />
              </div>
            </Tooltip>
            <Tooltip content="Second operand (not used for NOT, shifts)" position="right" className="w-full">
              <div>
                <label className="text-xs text-slate-500 mb-1 block">B (0–255)</label>
        ...
        <div className="space-y-2">
          <Tooltip content="Run visualization" position="top" className="w-full">
