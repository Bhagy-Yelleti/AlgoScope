...
export const CanvasFastExpo = ({ currentStep, hasSteps, inputBase, inputExp }) => {
  const bits = currentStep?.array ?? inputExp?.toString(2).split('').map(Number) ?? []
  ...
  const message = currentStep?.message ?? 'Enter base and exponent, then click Visualize.'
      ...
      <div className="rounded-xl border border-white/10 bg-slate-900/50 p-8 shadow-lg min-h-[350px] flex flex-col justify-center gap-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="rounded-xl bg-slate-800/60 p-5 border border-slate-700">
            ...
            <h2 className="text-3xl font-bold text-purple-400 mt-2 font-mono">{base}</h2>
          </div>
          <div className="rounded-xl bg-slate-800/60 p-5 border border-slate-700">
            <p className="text-slate-400 text-sm">Running Result</p>
            <h2 className="text-3xl font-bold text-cyan-400 mt-2 font-mono">{result}</h2>
          </div>
          <div className="rounded-xl bg-slate-800/60 p-5 border border-slate-700">
            <p className="text-slate-400 text-sm">
              {isComplete ? `${inputBase}^${inputExp}` : 'Final'}
            </p>
            <h2 className={`text-3xl font-bold mt-2 font-mono ${isComplete ? 'text-emerald-400' : 'text-slate-500'}`}>
              {isComplete ? result : '—'}
            </h2>
          </div>
        </div>
